...
async def nouveau_livreur(data: LivreurCreate, db: AsyncSession = Depends(get_db)):
...
async def liste_livreurs(db: AsyncSession = Depends(get_db)):
    """Liste tous les livreurs actifs"""
    return await lister_livreurs(db)

@router.get("/{id}", response_model=LivreurResponse)
async def detail_livreur(id: str, db: AsyncSession = Depends(get_db)):
    """Détail d'un livreur"""
...
@router.delete("/{id}")
async def supprimer_livreur(id: str, db: AsyncSession = Depends(get_db)):
