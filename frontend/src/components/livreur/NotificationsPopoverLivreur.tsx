...
function libelleDeclencheur(declencheur: string): string {
  return declencheur === "ASSIGNEE" ? "Commande assignée" : "Mise en livraison";
}

export default function NotificationsPopoverLivreur() {
  const { notifications, notificationsNonLues, marquerNotificationsCommeLues } = useLivreur();
    ...
    <Popover open={open} onOpenChange={handleOpenChange}>
      ...
      >
        ...
        {notificationsNonLues > 0 && (
          <span className="absolute -top-0.5 -right-0.5 min-w-[18px] h-[18px] px-1 rounded-full bg-[#DCA524] border-2 border-white text-[10px] font-bold text-white flex items-center justify-center">
      ...
      <PopoverContent
        align="end"
        className="w-96 p-0 rounded-2xl shadow-xl border-gray-100 overflow-hidden"
      >
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-gray-400 mt-1">
                    {formatHeure(n.createdAt)} · {n.telephone_destinataire}
                  </p>
                </div>
              </div>
            ))
